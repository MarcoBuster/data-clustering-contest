...
def clean_text(text):
    return [w for w in re.sub("[^A-Za-z ]+", "", text).lower().strip().split() if w not in ENGLISH_STOPWORDS and len(w) > 2]
...
def main():
    for filename in glob.glob(TRAIN_DATA + '/en_*.txt'):
        print(filename)
        with open(f'{filename}') as f:
            contents = f.read()
            ordered_ngrams = generate_ngrams(contents)
            with open(PROFILE_DATA + filename.lstrip(TRAIN_DATA).replace('txt', 'pickle'), 'wb+') as df:
